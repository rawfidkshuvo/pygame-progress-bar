...
button_y = progress_bar_y + progress_bar_height + 30

# Color button dimensions
color_button_radius = 45
color_button_y = progress_bar_y - color_button_radius - 70
color_button_x_offsets = [-300, -100, 100, 300]
...
labels = ["Rain", "Fog", "Snow", "Storm"]
statuses = ["Heavy", "Light", "Clear"]
color_selections = [green, green, green, green]
...
storm_images = [pygame.image.load(f"storm_{i}.png").convert_alpha() for i in range(3)]

# Load the car image
car_image = pygame.image.load("car.png").convert_alpha()
car_image = pygame.transform.scale(car_image, (60, 40))  # Resize the car image to fit the progress bar

# Create a list of lists containing the images
images = [rain_images, fog_images, snow_images, storm_images]
...
for condition_images in images:
    for i in range(len(condition_images)):
        condition_images[i] = apply_circular_mask(condition_images[i], 40)
...
def draw_round_button(x, y, radius, color, label, status, image):
    ...
    label_rect = label_surface.get_rect(center=(x, y + 70))
...
def draw_car(position):
    """Draw the car at the given x position on the progress bar."""
    # Ensure the car doesn't move beyond the progress bar's right edge
    car_x_position = min(position, progress_bar_x + progress_bar_width - car_image.get_width())
    car_y_position = progress_bar_y - car_image.get_height() + 5  # Adjust so bottom touches top of progress bar
    screen.blit(car_image, (car_x_position, car_y_position))
...
while running:
    ...
    # Draw car moving along the progress bar
    car_position = progress_bar_x + progress
    ...
    for i, offset in enumerate(color_button_x_offsets):
        ...
        draw_round_button(screen_width // 2 + offset, color_button_y, color_button_radius, color_selections[i], status_text, labels[i], current_image)
